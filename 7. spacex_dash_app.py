...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                dcc.Dropdown(id='site-dropdown',options=[{'label':'All Sites','value':'ALL'},
                                                                         ...
                                                                         value='ALL',
                                                                         placeholder="Just a Place Holder",
                                                                         searchable=True),
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        fig.update_layout(title='Total Success Launches for ' + entered_site)
        ...
        filtered_df = filtered_df.groupby(['Launch Site']  ,as_index =False).mean()
        filtered_df1 = filtered_df.copy()
        filtered_df1['class'] = filtered_df['class'].apply(lambda x:(1-x))
        filtered_df = pd.concat([filtered_df , filtered_df1] , axis = 0)
        filtered_df['Launch Site'] = ['Success', 'Fail']
